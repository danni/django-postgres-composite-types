...
class TestArrayFields(TestCase):
    ...
    def test_querying_array_field_contains(self):
        ...
        self.assertTrue(Hand.objects.filter(cards__contains=[queen_of_hearts]).exists())
        self.assertFalse(Hand.objects.filter(cards__contains=[jack_of_spades]).exists())
...
class TestNestedCompositeTypes(TestCase):
    ...
    def test_saving_and_loading_nested_composite_types(self):
        ...
        self.assertEqual(
            item.bounding_box,
            Box(top_left=Point(x=1, y=1), bottom_right=Point(x=4, y=2)),
        )

        self.assertEqual(item.bounding_box.bottom_left, Point(x=1, y=2))
        self.assertEqual(item.bounding_box.top_right, Point(x=4, y=1))
